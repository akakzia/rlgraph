...
for i in range(nb_seeds):
    for model in models:
        ...
        with open(job_file, 'w') as fh:
            ...
            fh.writelines("#SBATCH --job-name=ab_att_{}\n".format(model))
            fh.writelines("#SBATCH --qos=qos_gpu-t3\n")
            fh.writelines("#SBATCH --output=ab_att_{}%_%j.out\n".format(model))
            fh.writelines("#SBATCH --error=ab_att_{}%_%j.out\n".format(model))
            ...
            fh.writelines("srun python -u -B train.py --algo 'semantic' --n-blocks 5 --n-epochs 1000 --n-cycles 50 --n-batches 30 --architecture {} --save-dir 'attention_ablation_{}/' 2>&1 ".format(model, model))
