...
nb_seeds = 5
models = ['full_gn', 'relation_network', 'deep_sets', 'interaction_network_2']
test_ids = [2, 3]

for i in range(nb_seeds):
    for model in models:
        for id in test_ids:
            job_file = os.path.join(job_directory, f'tranfser_{model:s}_{id:d}%.slurm')
            ...
            with open(job_file, 'w') as fh:
                ...
                fh.writelines(f'#SBATCH --error=transfer_{model:s}_{id:d}%_%j.out\n')
